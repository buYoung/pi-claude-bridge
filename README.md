...
**Fast mode:** [fast mode](https://code.claude.com/docs/en/fast-mode) (research preview) runs the same Opus model at up to 2.5x output speed. Each model that supports it has a second picker entry: `claude-bridge/claude-fast-opus-5-5`, `claude-fast-opus-5` and `claude-fast-opus-4-8`, shown as e.g. "Claude Opus 5.5 Fast 1M". `claude-bridge/fast` selects the newest one. Existing shortcuts such as `claude-bridge/opus` keep selecting the standard models.

- **Cost:** fast mode is priced at $8/$40 per MTok input/output on Opus 5.5 and $10/$50 on Opus 5 and 4.8. On Pro, Max, Team and Enterprise it is paid from usage credits only, even when your plan still has usage left, so usage credits must be turned on at [claude.ai Settings > Usage](https://claude.ai/settings/usage). Anthropic API keys pay per token and need fast mode access provisioned. Bedrock, Vertex and Foundry do not offer it.
- **Only the fast entries use it:** `"fastMode": true` in `~/.claude/settings.json` (what Claude Code's `/fast` saves) has no effect through the bridge. Claude Code only serves fast mode to an Agent SDK host that asks for it, and the bridge asks only for a fast entry.
- **When it isn't served:** if Claude Code can't serve fast mode (no usage credits, disabled by your organization, `CLAUDE_CODE_DISABLE_FAST_MODE`, or the fast rate limit's cooldown), the turn still runs at standard speed and the bridge shows a notice once per cause.
- **Prompt cache:** fast mode sends a request header that is part of the cache key, so switching between a model's fast and standard entry mid-conversation re-reads the whole conversation uncached, at the new mode's rates. This happens on every switch, not just the first as with Claude Code's `/fast`, because the bridge starts a new Claude Code process for each prompt and the header does not carry over a resume. Choose the entry when you start a conversation.
- Compaction and branch summaries always run at standard speed, and AskClaude does not offer fast mode.
